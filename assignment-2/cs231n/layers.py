...
def batchnorm_forward(x, gamma, beta, bn_param):
    ...
    if mode == 'train':
        ...
        standard_deviation = np.sqrt(sample_var + eps)
        
        normalized_data = (x - sample_mean) / standard_deviation
...
def batchnorm_backward_alt(dout, cache):
    ...
    dx =(1 / N) * gamma * 1/sd * ((N * dout) - np.sum(dout, axis=0) - (x_minus_mean) * np.square(ivar) * np.sum(dout * (x_minus_mean), axis=0))
...
def conv_forward_naive(x, w, b, conv_param):
    ...
    out_H = np.int(1 + (H + 2 * P - HH) / S)
    out_W = np.int(1 + (W + 2 * P - WW) / S)
    
    out = np.zeros((N, F, out_H, out_W))
    for nn in range(N):
      for ff in range(F):
        for hh in range(0, out_H):
          for ww in range(0, out_W):
            out[nn, ff, hh, ww] = np.sum(x_pad[nn, :, hh*S:hh*S+HH, ww*S:ww*S+WW] * w[ff, :]) + b[ff]
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
    cache = (x, w, b, conv_param)
      ...
      for ff in range(F):
        for hh in range(out_H):
          for ww in range(out_W):
            dw[ff, :] += dout[nn, ff, hh, ww] * x_pad[nn,:,hh*S:hh*S+HH,ww*S:ww*S+WW]

    for nn in range(N):
      for ff in range(F):
        for hh in range(out_H):
          for ww in range(out_W):
            temp_dx[nn, :, hh*S:hh*S+HH,ww*S:ww*S+WW] += dout[nn, ff, hh, ww] * w[ff, ...]    
 
    dx = temp_dx[:, :, P:H+P, P:W+P]
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
    return dx, dw, db
...
def max_pool_forward_naive(x, pool_param):
    ...
    out_H = np.int(1 + (H - PH) / S)
    out_W = np.int(1 + (W - PW) / S)
      ...
      for c in range(C):  
        for h in range(dout_H): 
          for w in range(dout_W):  
            max_index = np.argmax(x[n, c, h*S:h*S+PH, w*S:w*S+PW])
            max_coord = np.unravel_index(max_index, [PH, PW])
            dx[n, c, h*S:h*S+PH, w*S:w*S+PW][max_coord] = dout[n, c, h, w]
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
    return dx
