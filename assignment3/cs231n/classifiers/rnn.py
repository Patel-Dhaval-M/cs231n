...
class CaptioningRNN(object):
    ...
    def loss(self, features, captions):
        ...
        ############################################################################
        cnn_to_rnn ,cache_cnn_to_rnn = affine_forward(features, W_proj, b_proj)
     
        embedded_captions, cache_word_embedding = word_embedding_forward(captions_in, W_embed)

        if self.cell_type == 'rnn':
          rnn_outputs, cache_rnn = rnn_forward(embedded_captions, cnn_to_rnn, Wx, Wh, b)
        elif self.cell_type == 'lstm':
          rnn_outputs, cache_rnn = lstm_forward(embedded_captions, cnn_to_rnn, Wx, Wh, b)
        ...
        dx_initial, grads["W_proj"], grads["b_proj"] = affine_backward(dh0, cache_cnn_to_rnn)
        ############################################################################
        #                             END OF YOUR CODE                             #
        ############################################################################
    ...
    def sample(self, features, max_length=30):
        ...
        for i in range(max_length):
          if self.cell_type == 'rnn':
            cur_next_state, _ = rnn_step_forward(word_embed, cur_hidden_state, Wx, Wh, b)
          elif self.cell_type == 'lstm':
            cur_next_state, cur_cell_state, _ = lstm_step_forward(word_embed, cur_hidden_state, cur_cell_state, Wx, Wh, b)

          cur_scores, _ = affine_forward(cur_next_state, W_vocab, b_vocab)
          captions[:, i] = np.argmax(cur_scores, axis = 1)
          word_embed, _ = word_embedding_forward(captions[:, i], W_embed)
          cur_hidden_state = cur_next_state
        ############################################################################
        #                             END OF YOUR CODE                             #
        ############################################################################
        return captions
