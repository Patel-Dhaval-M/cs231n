...
def rnn_forward(x, h0, Wx, Wh, b):
    ...
    for t in range(T):
      input_x = x[:, t, :]
      next_h, cache_temp = rnn_step_forward(input_x, prev_h, Wx, Wh, b)
      h[:, t, :] = next_h
      prev_h = next_h
...
def lstm_step_forward(x, prev_h, prev_c, Wx, Wh, b):
    ...
    cache = (x, Wx, Wh, b, prev_h, prev_c, i_gate, f_gate, o_gate, g_gate, next_c, next_h)
    ##############################################################################
    #                               END OF YOUR CODE                             #
    ##############################################################################
...
def lstm_step_backward(dnext_h, dnext_c, cache):
    ...
    #############################################################################
    x, Wx, Wh, b, prev_h, prev_c, i_gate, f_gate, o_gate, g_gate, next_c, next_h = cache
    ...
    dprev_c = dtanh_dc * f_gate
 
    da_i_partial = dtanh_dc * g_gate         
    da_g_partial = dtanh_dc * i_gate
    da_f_partial = dtanh_dc * prev_c

    da_i = i_gate * (1 - i_gate) * da_i_partial
    da_f = f_gate * (1 - f_gate) * da_f_partial
    da_o = o_gate * (1 - o_gate) * da_o_partial
    da_g = (1 - np.square(g_gate)) * da_g_partial
